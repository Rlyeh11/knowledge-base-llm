...
def _load_qa_limit_config(workspace: str) -> dict:
    ...
    default_config = {
        ...
        "use_lightrag": True,  # 新增：是否使用 LightRAG
...
def qa_node(state: QAInput, config: RunnableConfig, runtime: Runtime[Context]) -> QAOutput:
    ...
    desc: 基于LightRAG的智能问答系统，支持向量检索和混合检索
    ...
    try:
        ...
        use_lightrag = qa_limit_config.get("use_lightrag", True)  # 默认使用 LightRAG
        ...
        if use_lightrag:
            try:
                ...
                for doc in relevant_docs:
                    ...
                    doc_id = metadata.get("doc_id", metadata.get("file_path", "unknown"))
                    qa_sources.append(doc_id)
                ...
                with open(qa_file_path, 'w', encoding='utf-8') as f:
                    ...
                    f.write(f"**检索模式**: hybrid (LightRAG)\n\n")
                    f.write("---\n\n")
                    f.write("## 答案\n\n")
                    f.write(answer)
                    f.write("\n\n---\n\n")
                    f.write("## 来源文档\n\n")
                    for doc_id in qa_sources:
                        f.write(f"- [{doc_id}](javascript:void(0))\n")
                ...
                print("Warning: LightRAG not available, falling back to keyword retrieval")
                use_lightrag = False
            except Exception as e:
                print(f"Warning: LightRAG failed, falling back to keyword retrieval: {e}")
                use_lightrag = False

        # 如果不使用 LightRAG 或 LightRAG 失败，使用原有的关键词检索
        if not use_lightrag:
            import glob
            ...
            with open(qa_file_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"**检索模式**: keyword (fallback)\n\n")
                f.write("---\n\n")
                f.write("## 答案\n\n")
                f.write(answer)
                f.write("\n\n---\n\n")
